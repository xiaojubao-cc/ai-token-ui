...
import { getUserDashboardStats } from '@/api/dashboard'
...
async function loadStats() {
  ...
  try {
    ...
    const res = await getUserDashboardStats({ startTime, endTime })
...
<template>
  <div v-loading="loading">
    ...
    <el-row :gutter="20" class="chart-row">
      ...
      <el-col :span="12">
        <el-card shadow="hover">
          <template #header>各 API Key 用量分布</template>
    ...
    <el-row :gutter="20" class="chart-row">
      <el-col :span="24">
        <el-card shadow="hover">
          <template #header>API Key 用量对比</template>
